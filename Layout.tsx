...
export const Layout: React.FC<LayoutProps> = ({ 
  ...
  const menuItems = [
    ...
    { id: 'maintenance-history', label: 'Maintenance History', icon: HistoryIcon },
          ...
          {menuItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-4 px-3 py-3.5 rounded-2xl transition-all relative ${
                  isActive 
              ...
              >
                <item.icon size={20} className={isActive ? 'text-white' : 'text-slate-600 dark:text-slate-400'} />
                {(isSidebarOpen || window.innerWidth < 768) && (
                  <span className="flex-1 text-left whitespace-nowrap overflow-hidden text-ellipsis uppercase text-[10px] font-black tracking-wider">{item.label}</span>
                )}
                {(isSidebarOpen || window.innerWidth < 768) && item.hasStar && (
                  <Star size={14} className={`${isActive ? 'text-blue-200 fill-blue-200' : 'text-amber-500 fill-amber-500'} animate-pulse ml-auto`} />
                )}
